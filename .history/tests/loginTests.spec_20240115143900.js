...
async function getImageUrl(page, imgAltText) {
      const imageElement = await page.locator(`img[alt="${imgAltText}"]`);

      return imageUrl = await imageElement.getAttribute("src");
}
...
test('Login with problem user', async ({ page }) => {
      ...
      console.log(getImageUrl(page, "Sauce Labs Backpack"));
