...
test('Login with problem user', async ({ page }) => {
  ...
  await expect(getImageUrlAttribute(page, "Sauce Labs Backpack").toString()).toBe(
...
async function getImageUrlAttribute(page, imageAlt) {
  ...
  await page.waitForSelector(`img[alt="${imageAlt}"]`, { timeout: 5000 });

  const imageSrc = page.$eval(`img[alt="${imageAlt}"]`, (img) => img.src);
