...
test('Login with problem user', async ({ page }) => {
  ...
  await getImageUrlAttribute(page, "Sauce Labs Backpack");

  await expect(getImageUrlAttribute(page, "Sauce Labs Backpack")).toContain(
...
async function getImageUrlAttribute(page, imageAlt) {
  ...
  console.log(await page.$eval(selector, (img) => img.src));
