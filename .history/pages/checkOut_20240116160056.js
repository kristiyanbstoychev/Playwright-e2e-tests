...
exports.Checkout = class Checkout {
  ...
  constructor(page) {
    ...
    this.itemName = this.page.locator(".inventory_item_name:nth-child(1)");
  ...
  async verifyItem() {
    ...
    await expect(this.itemImageUrl).toContainText(
      this.backPackItem.itemImageUrl
    );
  }

  async checkOutFromProductPage() {
    await yourPage.clickOnFirstElementWithClassName(className);
  ...
  async clickOnFirstElementWithClassName(className) {
    await this.page.click(`.${className}:nth-child(1)`);
  }
};
