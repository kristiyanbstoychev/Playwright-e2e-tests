...
exports.Checkout = class Checkout {
  ...
  async verifyCheckoutFlow() {
    ...
    await expect(this.itemImageUrl).toHaveAttribute(
      "src",
  ...
  async checkOutFromProductPage() {
    await this.page.locator(
      "//div[@class='inventory_item_name ' and contains(text(), 'Backpack')]"
    ).click();

    await expect(this.itemImageUrl).toHaveAttribute("src", this.backPackItem.itemImageUrl);
