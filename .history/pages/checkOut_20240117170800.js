...
exports.Checkout = class Checkout {
  ...
  async checkoutFromTheMainPage() {
    await this.verifyItem("mainPage", this.bikeLightItem);
    await expect(this.itemImageUrl).toBeVisible();

    await this.verifyCheckoutFlow(this.bikeLightItem);
  }

  async checkOutFromProductPage(itemObject) {

    await this.page.locator("//img[@alt='"+ itemObject +"']").click();

    await expect(
      this.page.locator("//img[@alt='" + itemObject + "']")
    ).toBeVisible();
    await expect(this.backToProductsButton).toBeVisible();
    await this.verifyItem("itemPage", this.bikeLightItem);
